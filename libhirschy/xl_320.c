...
uint8_t build_frame(uint8_t id, uint8_t instruction, uint8_t *params, uint8_t nb_params, uint8_t buffer[XL_320_BUFFER_SIZE]){
  //Vérification des arguments
  if(id == 0xFD || id == 0xFF || params == 0 || buffer == 0){
  ...
  *(p_buffer++) = instruction;
  ...
  //Packet length
  uint16_t packet_length = (p_buffer-buffer) - 7 + 2;//Ce qu'on a écris - (header+id+length) + crc
